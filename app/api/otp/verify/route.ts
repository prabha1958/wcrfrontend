...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Verifying OTP request to backend:", body)
    console.log("[v0] Using endpoint:", API_ENDPOINTS.OTP_VERIFY)
      ...
      return NextResponse.json(
        {
          ...
          details: errorText,
          endpoint: API_ENDPOINTS.OTP_VERIFY,
        },
    ...
    console.log("[v0] Backend OTP Verify Response:", data)
    return NextResponse.json(data, { status: response.status })
  } catch (error) {
    console.error("[v0] OTP Verify API Error:", error)
    ...
    if (error instanceof Error) {
      if (error.name === "AbortError") {
        ...
        errorDetails = "The backend server at " + API_ENDPOINTS.OTP_VERIFY + " did not respond within 10 seconds"
      } else if (error.message.includes("fetch")) {
        errorMessage = "Cannot connect to backend server"
        errorDetails = "Make sure your Laravel server is running on " + API_ENDPOINTS.OTP_VERIFY
      } else {
    ...
    return NextResponse.json(
      {
        ...
        troubleshooting: [
          ...
          "3. Check if the endpoint /otp/verify exists in your Laravel routes",
